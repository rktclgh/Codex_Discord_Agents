...
3. Role Runner
PM, Backend Lead, QA, Research 같은 역할별 런타임입니다. 각 역할은 자기 역할에 맞는 응답과 작업 흐름을 유지합니다.
...
AGENT_TEAM_CODEX_ENABLE_SEARCH=1
AGENT_TEAM_CODEX_MODEL=gpt-5.4
AGENT_TEAM_CODEX_TIMEOUT_SECONDS=0
...
- 최신 라이브러리, 공식 문서, 외부 비교 자료를 묻는 전용 채널입니다.
- 기본적으로 이 채널의 사용자 메시지는 `Research` 역할에게 들어갑니다.
...
- 역할 에이전트가 실제로 작업할 프로젝트 루트 경로입니다.
- 이 경로 아래의 `AGENTS.md`, 코드, 스크립트, git 상태를 기준으로 Codex가 움직입니다.
- 예: `/Users/your-name/Desktop/MyRealProject`
...
`AGENT_TEAM_CODEX_ENABLE_SEARCH`

- `1`이면 기존 역할 전체와 `Research` 역할에 웹 검색을 허용합니다.
- 최신 라이브러리, 공식 문서, changelog, 외부 비교 조사에 사용됩니다.

`AGENT_TEAM_CODEX_MODEL`

- 역할별 Codex 세션이 사용할 모델입니다.
- 기본값은 `gpt-5.4`입니다.

`AGENT_TEAM_CODEX_TIMEOUT_SECONDS`

- 역할별 응답 생성 타임아웃입니다.
- `0`이면 타임아웃 없이 오래 걸리는 작업도 계속 진행합니다.
...
- `#router`: 전체 공유/이슈 관찰용
- `#pm`: PM과 직접 대화
- `#communication`: PM 최종 보고만 보는 조용한 채널
- `#backend`: 백엔드 리드/개발 관련
- `#frontend`: 프론트엔드 리드/개발 관련
- `#qa`: QA 관련
- `#security`: 보안 관련
- `#research`: 최신 자료/외부 조사 관련
...
자연어 요청을 보내면 먼저 "요청 확인 / 처리 중" 안내가 즉시 올라오고, 실제 Codex 작업이 끝나면 최종 결과가 이어서 올라옵니다.
...
또한 PM이 최종 정리 응답을 보낼 때는 오너 멘션이 함께 붙도록 설정할 수 있습니다.
